        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <form method="POST" action="{{ route('doencas.update', $doenca) }}" class="space-y-4">
                        @csrf
                        @method('PUT')

                        <div>
                            <label class="block text-sm mb-1">Nome *</label>
                            <input name="nome" value="{{ old('nome', $doenca->nome) }}"
                        ...
                        <div>
                            ...
                            <input name="codigo" value="{{ old('codigo', $doenca->codigo) }}"
                                   class="w-full rounded border-gray-300 dark:border-gray-700 dark:bg-gray-900">
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="descricao" rows="4"
                                      class="w-full rounded border-gray-300 dark:border-gray-700 dark:bg-gray-900">{{ old('descricao', $doenca->descricao) }}</textarea>
                        </div>

                        <div class="flex items-center gap-2">
                            <input id="ativa" type="checkbox" name="ativa" value="1"
                                   class="rounded border-gray-300 dark:border-gray-700"
                                   {{ old('ativa', $doenca->ativa) ? 'checked' : '' }}>
                    ...
                    <form class="mt-4" method="POST" action="{{ route('doencas.destroy', $doenca) }}"
