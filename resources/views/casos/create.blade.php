                    ...
                    <form method="POST" action="{{ route('casos.store') }}" class="space-y-4">
                        ...
                        <div>
                            ...
                            <select name="doenca_id" class="w-full rounded border-gray-300 dark:border-gray-700 dark:bg-gray-900" required>
                                ...
                                @foreach($doencas as $d)
                                    <option value="{{ $d->id }}" {{ old('doenca_id') == $d->id ? 'selected' : '' }}>
                                        {{ $d->nome }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="unidade_saude_id" class="w-full rounded border-gray-300 dark:border-gray-700 dark:bg-gray-900" required>
                                ...
                                @foreach($unidades as $u)
                                    <option value="{{ $u->id }}" {{ old('unidade_saude_id') == $u->id ? 'selected' : '' }}>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="date" name="data_notificacao" value="{{ old('data_notificacao', now()->toDateString()) }}"
                            ...
                            <div>
                                ...
                                <select name="estado" class="w-full rounded border-gray-300 dark:border-gray-700 dark:bg-gray-900" required>
                                    @foreach(['SUSPEITO','CONFIRMADO','DESCARTADO','OBITO'] as $e)
                                        <option value="{{ $e }}" {{ old('estado','SUSPEITO') === $e ? 'selected' : '' }}>
                                            {{ $e }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="number" name="idade" min="0" max="120" value="{{ old('idade') }}"
                            ...
                            <div>
                                ...
                                <select name="sexo" class="w-full rounded border-gray-300 dark:border-gray-700 dark:bg-gray-900" required>
                                    <option value="N" {{ old('sexo','N') === 'N' ? 'selected' : '' }}>Não informado</option>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input name="paciente_iniciais" value="{{ old('paciente_iniciais') }}"
                                       class="w-full rounded border-gray-300 dark:border-gray-700 dark:bg-gray-900"
                                       placeholder="Ex: A.M.">
                            </div>
                            <div>
                                <label class="block text-sm mb-1">Telefone de contacto</label>
                                <input name="telefone_contacto" value="{{ old('telefone_contacto') }}"
                                       class="w-full rounded border-gray-300 dark:border-gray-700 dark:bg-gray-900">
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="observacoes" rows="4"
                                      class="w-full rounded border-gray-300 dark:border-gray-700 dark:bg-gray-900">{{ old('observacoes') }}</textarea>
                        </div>
